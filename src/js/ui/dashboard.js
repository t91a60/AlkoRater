...
function getSmartGreeting(total) {
    ...
    if (h >= 0 && h < 6) {
        return {
            emoji: '🌙',
            title: 'Nocna degustacja?',
            sub: 'Trunki nie śpią, ale ty powinieneś.',
        };
    }
    if (h >= 6 && h < 10) {
        return { emoji: '☀️', title: 'Dzień dobry', sub: 'Gotowy na nowe odkrycia?' };
    }
    if (h >= 10 && h < 12) {
        return { emoji: '🥂', title: 'Pora na coś dobrego', sub: 'Twoja kolekcja czeka.' };
    }
    if (h >= 12 && h < 17) {
        return {
            emoji: '🍺',
            title: 'Popołudniowa okazja',
            sub: total > 0 ? `Masz już ${total} ocen. Nie zwalniaj.` : 'Zacznij swoją kolekcję.',
        };
    }
    if (h >= 17 && h < 20 && isWeekend) {
        return {
            emoji: '🎉',
            title: 'Weekendowy wieczór',
            sub: 'Najlepszy czas na nowego odkrycia.',
        };
    }
    if (h >= 17 && h < 20) {
        return { emoji: '🌆', title: 'Wieczorne odkrycia', sub: 'Coś nowego na dziś?' };
    }
    return {
        emoji: '🌃',
        title: 'Dobry wieczór',
        sub: isWeekend
            ? 'Weekend, czas na coś wyjątkowego.'
            : 'Spokojny wieczór z dobrym trunkiem.',
    };
}
...
export function updateDashboard() {
    ...
    state.favorites.forEach((fav) => {
        ...
        totalScore += stars;
        if (!bestEntry || stars > parseInt(bestEntry.stars, 10)) {
            bestEntry = fav;
        }
    });
...
export function updateRecentlyRated() {
    ...
    if (recent.length === 0) {
        container.innerHTML =
            '<p style="opacity:0.38;font-size:13px;padding:10px 0;font-weight:400;">Oceń pierwszy trunek, aby zobaczyć go tutaj.</p>';
        return;
    }
